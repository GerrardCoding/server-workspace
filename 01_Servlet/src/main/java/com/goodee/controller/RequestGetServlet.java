...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 *  
		 *  첫번째 메서드 매개변수인 request에는 요청시 담긴 내용이 저장돼 있음.
		 *  두번째 메서드 매개변수인 response에는 요청처리후 응답할 때 사용될 객체(응답할 값들을 저장함.)
		 * 
		 *  따라서 request의 parameter영역으로 부터 전달된 데이터를 추출하는 메서드는 다음과 같음.
		 *  	> request.getParameter("키(name)"); => String 타입으로 전달됨. 만약 전달한값이 숫자일 경우 형변환 해야함.
		 ...
		 * 즉, 여기 자바코드내에 사용자가 보게 될 응답 HTML 구문 작성하여 응답함.
		 * 
		 * 장점 : JAVA 코드내에 작성하기 때문에 자바내에서 반복문이나 조건문, 유용한 메서드 등을 바로 활용할 수 있음.
